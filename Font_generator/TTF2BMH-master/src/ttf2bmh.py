        ...
        for ttf_file in TTF_FILES:
            ...
            for height_idx in height_indices:
                ...
                for char in chars:
                    ...
                    write_bmh_char(outfile, char, dot_array, progmem, filename)
                    if(print_ascii):
                        print(char + ":")
                        print_char(image, height, char_width, x_offset)

                # write tail and close bmh file
                write_bmh_tail(outfile, width_array, character_line, filename)
...
#---------------------------------------------------------------------------------------
#
def write_bmh_char(outfile, char, dot_array, progmem, filename):
    # C Type declaration strings
    # Adjust for different MCU/compilers
    C_declaration_0 = 'const char ' + filename + '_'
    if(progmem):
...
def write_bmh_tail(outfile, width_array, character_line, filename):
    C_addr_array = []
    C_char_width_0 = 'const char ' + filename + '_width[] = {'
    C_char_width_1 = (','.join(width_array))
    C_char_width_2 = '};\n'

    outfile.write(C_char_width_0 + C_char_width_1 + C_char_width_2)
    C_addr_array.append(filename + "_width")
    for char in character_line:
        C_addr_array.append(filename + '_' + str(ord(char)))

    C_addr  = (', '.join(C_addr_array))
    C_address_declaration_1 = "const char* " + filename + "_addr[] = {"
